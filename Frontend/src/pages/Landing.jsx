        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8 z-10 relative flex flex-col items-start text-left">
            ...
            <h1 className="text-5xl sm:text-7xl font-bold font-serif leading-[0.95] tracking-tight">
              Dime qué <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">coleccionas</span> <br/>
              y te diré quién eres.
            </h1>
            
            <p className="text-xl text-base-content/70 max-w-md leading-relaxed">
              El espacio para curar tus obsesiones. Libros, películas, vinilos o cafeterías. Organiza tu mundo y descubre el de otros.
            </p>

            <div className="flex flex-wrap gap-4 pt-2">
              <Link to="/register" className="btn btn-primary btn-lg rounded-full px-8 gap-3 group shadow-lg shadow-primary/20">
                Empezar mi colección 
                <ArrowRight className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/explore" className="btn btn-outline btn-lg rounded-full px-8">
                Explorar
              </Link>
            </div>

            {/* Social Proof */}
            <div className="flex items-center gap-4 pt-6 opacity-60 text-sm">
              <div className="flex -space-x-2">
                 {[1,2,3,4].map(i => (
                   <div key={i} className="w-8 h-8 rounded-full border-2 border-base-100 bg-gray-300 overflow-hidden">
                     <img src={`https://i.pravatar.cc/100?img=${i+10}`} alt="user" />
                   </div>
                 ))}
              </div>
              <p>+2.000 curadores activos</p>
            </div>
          </div>
            ...
            <div className="absolute top-0 left-0 w-full h-full grid grid-cols-2 gap-4 rotate-12 opacity-80 -translate-y-20 translate-x-10 scale-110">
               ...
               <div className="flex flex-col gap-4 animate-[scrollY_20s_linear_infinite]">
                 {[...covers, ...covers].map((src, i) => (
                   <div key={`c1-${i}`} className="w-full aspect-[3/4] rounded-xl overflow-hidden shadow-2xl bg-base-200">
                     <img src={src} alt="cover" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-500" />
                   </div>
                 ))}
               </div>
               {/* Columna 2 */}
               <div className="flex flex-col gap-4 -mt-32 animate-[scrollY_25s_linear_infinite_reverse]">
                 {[...covers, ...covers].reverse().map((src, i) => (
                   <div key={`c2-${i}`} className="w-full aspect-[3/4] rounded-xl overflow-hidden shadow-2xl bg-base-200">
                      <img src={src} alt="cover" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-500" />
                   </div>
                 ))}
               </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="card bg-base-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="card-body">
                ...
                <h3 className="text-xl font-bold font-serif">Organiza Visualmente</h3>
                <p className="opacity-70">Olvida las hojas de cálculo. Tus libros y películas merecen verse tan bien como en tu estantería.</p>
              </div>
            </div>
            <div className="card bg-base-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="card-body">
                <Users className="w-10 h-10 text-primary mb-2" />
                <h3 className="text-xl font-bold font-serif">Comunidad de Nicho</h3>
                <p className="opacity-70">Sigue a personas con gustos afines, no algoritmos que te venden cosas.</p>
              </div>
            </div>
            <div className="card bg-base-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="card-body">
                <Star className="w-10 h-10 text-primary mb-2" />
                <h3 className="text-xl font-bold font-serif">Descubrimiento Real</h3>
                <p className="opacity-70">Encuentra tu próxima obsesión a través de las colecciones de expertos y amigos.</p>
              </div>
            </div>
          </div>
        </div>
