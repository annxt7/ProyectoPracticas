...
const Profile = () => {
  ...
  const myId = user?.userId || user?.user_id;
  const targetId = userId ? userId : myId;
  const isOwnProfile = user && String(targetId) === String(myId);
  ...
  const handleFileUpload = async (e, type) => {
    ...
    try {
      ...
      const payload =
        type === "avatar" ? { avatarUrl: cloudUrl } : { bannerUrl: cloudUrl };
          ...
          <div className="px-6 relative">
            <div className="flex justify-between items-end -mt-12 mb-4">
              ...
              <div
                className="relative"
                onClick={() =>
                  isOwnProfile && !isUploading && avatarInputRef.current.click()
                }
              >
                <div
                  className={`avatar ring-4 ring-base-100 rounded-full bg-base-100 shadow-sm ${
                    isOwnProfile ? "cursor-pointer hover:ring-primary/50" : ""
                  }`}
                >
            ...
            <div className="space-y-3 mb-6">
              ...
              {isEditing ? (
                <form
                  ...
                  className="flex flex-col md:flex-row gap-3 items-start max-w-xl"
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 md:px-6 min-h-[300px]">
          ...
          {isOwnProfile && activeTab === "collections" && (
            <Link
              ...
              className="aspect-4/5 bg-base-100 border-2 border-dashed border-base-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-base-200/50 hover:border-primary/50 transition-all group"
            >
              ...
              >
                <div className="card bg-base-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 rounded-2xl overflow-hidden aspect-4/5 relative">
                  ...
                  <img
                    src={
                      col.cover_url ||
                      `https://picsum.photos/400?random=${col.collection_id}`
                    }
