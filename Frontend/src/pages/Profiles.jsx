...
const Profile = () => {
  ...
  // Efecto para sincronizar datos del usuario logueado
  useEffect(() => {
    if (isMe && user) {
      setProfileData(user);
      ...
      <main className="mx-auto">
            ...
            <div className="flex gap-2 mb-2">
              {isMe ? (
                <>
                  ...
                  {/* Botón que abre el Modal de Configuración */}
                  <button 
            ...
            {isEditing ? (
              <form onSubmit={handleSaveBio} className="flex flex-col gap-2 max-w-xl">
                ...
                <div className="flex justify-end gap-2">
                  <button type="submit" className="btn btn-primary btn-sm btn-square"><Check size={18} /></button>
        ...
        <div className="border-t border-white/10 mt-4 sticky top-16 bg-base-100/95 z-30 flex justify-center gap-12 backdrop-blur-md">
          <button 
            ...
            className={`py-4 border-b-2 px-4 text-sm font-bold transition-all ${activeTab === "collections" ? "border-primary text-primary" : "border-transparent opacity-50"}`}
          >
            COLECCIONES
          </button>
          {isMe && (
            <button 
              ...
              className={`py-4 border-b-2 px-4 text-sm font-bold transition-all ${activeTab === "saved" ? "border-primary text-primary" : "border-transparent opacity-50"}`}
            >
              GUARDADO
            </button>
          )}
