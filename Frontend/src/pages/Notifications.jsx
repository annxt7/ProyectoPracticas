...
const Activity = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // 1. CORRECCIÓN DE RUTA: Según tu server.js, la ruta es /api/users/me
      // Esto elimina el error 404 de "api/auth/me"
      const userRes = await api.get("/users/me");
      setCurrentUser(userRes.data);

      // 2. CORRECCIÓN DE RUTA: Según tu server.js, la ruta es /api/activity
  ...
  const markAsRead = async (id) => {
    ...
    try { 
      // Coincide con app.use('/api/activity', activityRoutes)
    ...
    <div className="min-h-screen pb-28 md:pb-10 bg-base-100 text-white font-sans">
      ...
      {currentUser && (
        <div className="bg-primary/10 text-primary text-[10px] py-1 text-center border-b border-primary/20 tracking-widest uppercase font-bold">
          Sincronizado: {currentUser.username} (ID: {currentUser.id})
        </div>
      )}
        ...
        <main className="max-w-2xl w-full mx-auto">
          {loading ? (
            ...
            <div className="text-center py-24 bg-white/[0.02] rounded-[3rem] border border-dashed border-white/10">
              ...
              <p className="opacity-40 text-sm italic">No hay actividad para mostrar</p>
            </div>
          )}
        </main>
  ...
  return (
    <div 
      ...
      className={`flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300 ${
        data.read ? "bg-transparent border-transparent opacity-40" : "bg-white/[0.05] border-white/10 hover:bg-white/[0.08] cursor-pointer"
      }`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className="text-[10px] opacity-30 uppercase">{new Date(data.created_at).toLocaleDateString()}</span>
      </div>

      {!data.read && <div className="w-2.5 h-2.5 rounded-full bg-primary animate-pulse shadow-[0_0_10px_#570df8]"></div>}
    </div>
  );
};
