...
import { Search, X, UserPlus, Check, ChevronRight, Home, Heart, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-transparent pb-24 md:pb-10 font-sans text-base-content">
      ...
      <div className="sticky top-0 md:top-20 z-40 pt-4 pb-0 shadow-sm border-b border-base-200">
         <div className="max-w-2xl mx-auto px-4 md:px-0">
            ...
            <div className="relative mb-4 border-base-100 shadow-amber-100/50">
               ...
               />
               {query && (
                 <button onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 btn btn-circle btn-xs btn-ghost">
                   <X size={16} />
                 </button>
               )}
            </div>
         ...
         {activeTab === 'cuentas' && (
           <div className="flex flex-col gap-2">
             {filteredUsers.length > 0 ? filteredUsers.map((user) => (
               <div key={user.id} className="flex items-center justify-between p-3 hover:bg-base-200/50 rounded-xl transition-colors cursor-pointer group">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                       ...
                       <p className="text-xs opacity-50 mt-1 truncate max-w-[200px]">{user.bio}</p>
                     </div>
                  </div>
               ...
               <div className="text-center py-10 opacity-50">No se encontraron usuarios</div>
             )}
           </div>
         )}
         ...
         {activeTab === 'colecciones' && (
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             {filteredCollections.length > 0 ? filteredCollections.map((col) => (
               <div key={col.id} className="card bg-base-100 shadow-sm border border-base-200 hover:shadow-md transition-shadow cursor-pointer overflow-hidden group">
                  <div className="flex gap-3 p-3">
                     {/* Mini Cover */}
                     <div className="w-20 h-20 rounded-lg bg-base-200 overflow-hidden flex-shrink-0">
                       <img src={col.cover} alt="cover" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                     </div>
                     {/* Info */}
                     <div className="flex flex-col justify-center flex-1 min-w-0">
                       <h3 className="font-bold font-serif text-lg truncate leading-tight">{col.title}</h3>
                       <p className="text-xs opacity-50 mt-1">{col.items} elementos</p>
                       <div className="flex items-center gap-1.5 mt-2">
                          <div className="w-4 h-4 rounded-full bg-primary/20 overflow-hidden">
                             <img src={`https://i.pravatar.cc/100?u=${col.id}`} alt="creator" />
                          </div>
                          <span className="text-xs font-medium opacity-70">{col.author}</span>
                       </div>
                     </div>
               ...
               <div className="col-span-full text-center py-10 opacity-50">No se encontraron colecciones</div>
             )}
           </div>
         )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-base-100 border-t border-base-200 px-6 py-3 md:hidden z-50">
        <div className="flex justify-between items-center max-w-sm mx-auto">
          <NavLinkMobile icon={<Home size={24} />} page={'/'} />
