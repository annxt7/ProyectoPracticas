    ...
    <div className="fixed inset-0 z-999 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      ...
      <div className="bg-base-100 text-base-content border border-base-300 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1 bg-linear-to-b from-transparent to-base-200/20">
            ...
            <div className="collapse-content px-5 pb-5">
              <div className="grid grid-cols-3 gap-3 pt-2">
                {availableThemes.map((t) => (
                  ...
                  >
                    <div className={`w-full h-10 rounded-lg ${t.bg} border border-base-300 overflow-hidden relative shadow-sm`}>
                      <div className={`absolute top-0 left-0 w-1/3 h-full ${t.primary} opacity-80`}></div>
                    </div>
                    <span className="text-[9px] font-bold uppercase tracking-wider opacity-70">{t.name}</span>
                    {theme === t.id && (
          ...
          <div className="collapse collapse-plus bg-base-200/50 rounded-3xl border border-base-300">
            ...
            <div className="collapse-content px-5 pb-5 text-[11px] opacity-70 leading-relaxed">
              <p className="mb-4">
                <strong className="text-base-content">Tribe</strong> nació con una misión clara: organizar el caos digital. Creemos que el valor reside en la selección.
              </p>
              <div className="grid grid-cols-2 gap-3 py-2">
                <div className="p-3 rounded-2xl bg-base-100 border border-base-300">
                  <h5 className="text-[10px] font-bold uppercase mb-1">La Visión</h5>
                  <p className="text-[9px] opacity-50">Tu catálogo de pasiones.</p>
                </div>
                <div className="p-3 rounded-2xl bg-base-100 border border-base-300">
                  <h5 className="text-[10px] font-bold uppercase mb-1">Comunidad</h5>
                  <p className="text-[9px] opacity-50">Conexión real sin algoritmos.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="collapse-content px-5 pb-5 space-y-3">
              <button className="btn btn-ghost bg-base-100 btn-block rounded-2xl h-auto py-3 flex-col gap-1 hover:bg-error/10 hover:text-error border-none group transition-all">
                <div className="flex items-center gap-2 font-bold text-xs">
                  <Coffee size={18} className="text-warning group-hover:animate-bounce" /> Invitarnos a un café
                </div>
        ...
        <div className="p-6 bg-base-300/50 text-center">
          <p className="text-[9px] opacity-30 uppercase tracking-[0.3em] font-bold italic">Design by Tribe Team • 2026</p>
        </div>
      </div>
    </div>
  );
